...
public class AdminMenuCreatedEventConsumer : IConsumer<AdminMenuEvent>
{
	...
	public async Task HandleEventAsync(AdminMenuEvent createdEvent)
	{
		...
		nopStationAdminMenuItem2.Title = await _localizationService.GetResourceAsync("Plugins.Widgets.Product360View.Menu.Product360View");
		...
		if (await _permissionService.AuthorizeAsync("ManageNopStationProduct360View"))
		{
			AdminMenuItem adminMenuItem = new AdminMenuItem();
			AdminMenuItem adminMenuItem2 = adminMenuItem;
			adminMenuItem2.Title = await _localizationService.GetResourceAsync("Plugins.Widgets.Product360View.Menu.Configuration");
			adminMenuItem.Url = "~/Admin/Product360View/Configure";
			adminMenuItem.Visible = true;
			adminMenuItem.IconClass = "far fa-circle";
			adminMenuItem.SystemName = "Product360View.Configuration";
		...
		if (menuItem.ChildNodes.Any())
		{
			if (await _permissionService.AuthorizeAsync("ShowNopStationDocumentations"))
			{
				...
				adminMenuItem2.Url = "https://www.nop-station.com/product360view-documentation?utm_source=admin-panel?utm_source=admin-panel&utm_medium=products&utm_campaign=product360view";
