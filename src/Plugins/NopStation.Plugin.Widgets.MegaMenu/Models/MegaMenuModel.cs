...
public record MegaMenuModel
{
	...
	public bool HasOnlyCategories
	{
		get
		{
			if (Categories.Any() && !Topics.Any() && !DisplayHomePageMenuItem && (!DisplayNewProductsMenuItem || !NewProductsEnabled) && !DisplayProductSearchMenuItem && !DisplayCustomerInfoMenuItem && (!DisplayBlogMenuItem || !BlogEnabled) && (!DisplayForumsMenuItem || !ForumEnabled))
			{
				return !DisplayContactUsMenuItem;
			}
			return false;
		}
	}
