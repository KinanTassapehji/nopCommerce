...
public class CategoryIconService : ICategoryIconService
{
	...
	public async Task<CategoryIcon> GetCategoryIconByCategoryIdAsync(int categoryId)
	{
		...
		return await _categoryIconRepository.Table.FirstOrDefaultAsync((CategoryIcon x) => x.CategoryId == categoryId);
	}

	public async Task<IPagedList<CategoryIcon>> GetAllCategoryIconsAsync(int pageIndex = 0, int pageSize = int.MaxValue)
	{
		return await _categoryIconRepository.Table.OrderByDescending((CategoryIcon e) => e.Id).ToPagedListAsync(pageIndex, pageSize);
	}
	...
	public IList<CategoryIcon> GetCategoryIconByIds(int[] categoryIconIds)
	{
		...
		return _categoryIconRepository.Table.Where((CategoryIcon x) => Enumerable.Contains(categoryIconIds, x.Id)).ToList();
	}
