...
public class AdminMenuCreatedEventConsumer : IConsumer<AdminMenuEvent>
{
	...
	public async Task HandleEventAsync(AdminMenuEvent createdEvent)
	{
		...
		nopStationAdminMenuItem2.Title = await _localizationService.GetResourceAsync("Admin.NopStation.CancelOrder.Menu.CancelOrder");
		...
		if (await _permissionService.AuthorizeAsync("ManageNopStationCancelOrder"))
		{
			AdminMenuItem adminMenuItem = new AdminMenuItem();
			AdminMenuItem adminMenuItem2 = adminMenuItem;
			adminMenuItem2.Title = await _localizationService.GetResourceAsync("Admin.NopStation.CancelOrder.Menu.Configuration");
			adminMenuItem.Url = "~/Admin/CancelOrder/Configure";
			adminMenuItem.Visible = true;
			adminMenuItem.IconClass = "far fa-circle";
			adminMenuItem.SystemName = "CancelOrder.Configure";
		...
		if (menuItem.ChildNodes.Any())
		{
			if (await _permissionService.AuthorizeAsync("ShowNopStationDocumentations"))
			{
				...
				adminMenuItem2.Url = "https://www.nop-station.com/cancel-order-documentation?utm_source=admin-panel&utm_medium=products&utm_campaign=cancel-order";
