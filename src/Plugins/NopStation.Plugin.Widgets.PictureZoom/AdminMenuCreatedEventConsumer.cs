...
public class AdminMenuCreatedEventConsumer : IConsumer<AdminMenuEvent>
{
	...
	public async Task HandleEventAsync(AdminMenuEvent createdEvent)
	{
		...
		nopStationAdminMenuItem2.Title = await _localizationService.GetResourceAsync("Admin.NopStation.PictureZoom.Menu.PictureZoom");
		...
		if (await _permissionService.AuthorizeAsync("ManageNopStationPictureZoom"))
		{
			AdminMenuItem adminMenuItem = new AdminMenuItem();
			AdminMenuItem adminMenuItem2 = adminMenuItem;
			adminMenuItem2.Title = await _localizationService.GetResourceAsync("Admin.NopStation.PictureZoom.Menu.Configuration");
			adminMenuItem.Url = "~/Admin/PictureZoom/Configure";
			adminMenuItem.Visible = true;
			adminMenuItem.IconClass = "far fa-circle";
			adminMenuItem.SystemName = "PictureZoom.Configuration";
		...
		if (menuItem.ChildNodes.Any())
		{
			...
			adminMenuItem2.Url = "https://www.nop-station.com/picture-zoom-documentation?utm_source=admin-panel&utm_medium=products&utm_campaign=picture-zoom";
