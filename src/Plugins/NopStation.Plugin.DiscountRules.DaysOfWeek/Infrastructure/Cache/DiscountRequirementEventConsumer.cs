...
namespace NopStation.Plugin.DiscountRules.DaysOfWeek.Infrastructure.Cache;
...
public class DiscountRequirementEventConsumer : IConsumer<EntityDeletedEvent<DiscountRequirement>>
{
	...
	public async Task HandleEventAsync(EntityDeletedEvent<DiscountRequirement> eventMessage)
	{
		...
		{
			Setting setting = await _settingService.GetSettingAsync($"DiscountRequirement.DaysOfWeek-{discountRequirement.Id}");
			if (setting != null)
			{
