...
public class CompletedOrderRule : BaseReminderRule
{
	private readonly IRepository<Order> _orderRepository;

	public override string SystemName => "CompletedOrder";

	public CompletedOrderRule(IRepository<Customer> customerRepository, IRepository<ReminderExcludedCustomer> reminderExcludedCustomerRepository, IRepository<Order> orderRepository)
	...
	public override async Task<IList<Customer>> GetEligibleCustomersAsync(Reminder reminder, int dateGreaterThanMinutes, int dateLowerThanMinutes)
	{
		...
		IQueryable<Order> source = _orderRepository.Table.Where((Order o) => o.OrderStatusId == 30 && !o.Deleted && o.CreatedOnUtc >= startDate && o.CreatedOnUtc <= endDate);
	...
	public override async Task<DateTime?> GetConditionMetDateAsync(Customer customer)
	{
		return await (from o in _orderRepository.Table
			where o.CustomerId == customer.Id && o.OrderStatusId == 30 && !o.Deleted
