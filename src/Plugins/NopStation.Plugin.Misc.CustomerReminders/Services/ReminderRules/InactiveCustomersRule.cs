...
public class InactiveCustomersRule : BaseReminderRule
{
	public override string SystemName => "InactiveCustomers";

	public InactiveCustomersRule(IRepository<Customer> customerRepository, IRepository<ReminderExcludedCustomer> reminderExcludedCustomerRepository)
	...
	public override async Task<IList<Customer>> GetEligibleCustomersAsync(Reminder reminder, int dateGreaterThanMinutes, int dateLowerThanMinutes)
	{
		...
		IQueryable<Customer> source = _customerRepository.Table.Where((Customer c) => !c.Deleted && c.LastActivityDateUtc >= startDate && c.LastActivityDateUtc <= endDate);
	...
	{
		return Task.FromResult((DateTime?)customer.LastActivityDateUtc);
	}
}
